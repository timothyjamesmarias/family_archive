...
- **Artifact files are Active Storage attachments** — `ArtifactFile` has
  ...
  anchor. Move to S3 by adding a service in `config/storage.yml`.
...
Active Storage writes to local disk, which is ephemeral on Render — add an S3
service to `config/storage.yml` before uploading anything you care about.
